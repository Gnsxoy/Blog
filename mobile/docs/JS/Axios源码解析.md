...
> **关于 `axios` 的源码解析，重点介绍了其调用方式、核心函数 `request` 和 `dispatchRequest` 的实现原理，并探讨了部分内置方法的实现。**
...
- **「 `axios` 为什么既可以作为对象，又可以作为函数本身来进行调用呢？ 」**

  **`axios` 其实就是 `bind` 函数执行后返回的 `wrap` 函数。**

  **它会拷贝 `axios` 的原型属性、方法以及实例属性和方法，挂载到 `instance` 上，也就是 `wrap`。**
...
- 拿到的是 `config` 对象，最终返回的是 `response` 结果。
  ...
  > 对 请求拦截器和响应拦截器 进行一个 对接请求以及发起请求 的作用。 😯 高级 ✨~
...
`dispatchRequest` 是利用 `Promise` 包装的 `XMLHttpRequest` 方法，对请求 `config` 和 `response` 数据结果进行处理。
